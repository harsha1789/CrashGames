...
for line in lines:
    if ".video_start = " in line or ".video_end = " in line or "context_start_time = time.time()" in line or "t_now = " in line:
        continue
    if line.strip().startswith("_t") and "_start = " in line:
        continue
...
for line in clean_lines:
    ...
    if 'results.append(t3)' in line:
        ...
        out.append(f"{indent}t3.video_start = _t3_start\n{indent}t_wager.video_start = _t3_start\n{indent}t_payout.video_start = _t3_start\n{indent}t_feat.video_start = _t3_start\n{indent}t_bal.video_start = _t3_start\n")
...
with open('test_spin_button.py', 'w', encoding='utf-8') as f:
    f.writelines(out)
print("Time tracking injected successfully with local variables!")
